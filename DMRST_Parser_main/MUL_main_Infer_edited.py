...
from .model_depth_edited import ParsingNet
...
def inference(model, tokenizer, input_sentences, batch_size, input_EDU_breaks = None):
    ...
    with torch.no_grad():
        for loop in range(LoopNeeded):
            ...
            _, _, SPAN_batch, _, predict_EDU_breaks, SPAN_logits = model.TestingLoss(input_sen_batch,
                                                                        input_EDU_breaks=input_EDU_breaks, LabelIndex=None,
                                                                        ParsingIndex=None, GenerateTree=True,
                                                                        use_pred_segmentation = True if not input_EDU_breaks else False)
            all_segmentation_pred.extend(predict_EDU_breaks)
            all_tree_parsing_pred.extend(SPAN_batch)
            all_tree_parsing_logits.extend(SPAN_logits)
    return input_sentences, all_segmentation_pred, all_tree_parsing_pred, all_tree_parsing_logits # Last ADDED
...
if __name__ == '__main__':
    ...
    for i in range(len(input_sentences)):
        ...
        print(all_segmentation_pred[i])
        print(all_tree_parsing_pred[i])
        print("-----------------------------\n")
