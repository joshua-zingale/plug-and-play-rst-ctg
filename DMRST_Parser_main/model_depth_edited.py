...
class ParsingNet(nn.Module):
    def __init__(self, language_model, word_dim=768, hidden_size=768, decoder_input_size=768,
                 ...
                 rnn_layers=1, dropout_e=0.5, dropout_d=0.5, dropout_c=0.5, bert_tokenizer=None, device = "cuda:0"):
        ...
        self.encoder = EncoderRNN(language_model, word_dim, hidden_size, config.enc_rnn_layer_num, dropout_e, bert_tokenizer=bert_tokenizer, segmenter=self.segmenter, device = device)
    ...
    def TestingLoss(self, input_sentence, input_EDU_breaks, LabelIndex, ParsingIndex, GenerateTree, use_pred_segmentation):
        ...
        for i in range(len(EDU_breaks)):
            ...
            if len(EDU_breaks[i]) == 1:
                ...
                #### ADDED ####
                SPAN_logits.append(relation_weights)
                ## END ADDED ##
        ...
        return Loss_tree_batch, Loss_label_batch, (SPAN_batch if GenerateTree else None), (merged_label_gold, merged_label_pred), EDU_breaks, (SPAN_logits if GenerateTree else None) # Last ADDED
