...
class EncoderRNN(nn.Module):
    def __init__(self, language_model, word_dim, hidden_size, rnn_layers, dropout, bert_tokenizer=None, segmenter=None, device = "cuda:0"):
    ...
    def forward(self, input_sentence, EDU_breaks, is_test=False):
        ...
        for output in tem_outputs:
            ...
            all_outputs.append(torch.cat([output, torch.zeros(1, max_edu_break_num - cur_break_num, self.word_dim).to(self.device)], dim=1))
...
class Segmenter_pointer(nn.Module):
    ...
    def __init__(self, hidden_size, atten_model=None, decoder_input_size=None, rnn_layers=None, dropout_d=None, device = "cuda:0"):
    ...
    def train_segment_loss(self, word_embeddings, edu_breaks):
        ...
        for step, start_index in enumerate(edu_breaks[:-1]):
            ...
            cur_ground_index = torch.tensor([edu_breaks[step + 1] - start_index]).to(self.device)
...
class Segmenter(nn.Module):
    def __init__(self, hidden_size, device = "cuda:0"):
